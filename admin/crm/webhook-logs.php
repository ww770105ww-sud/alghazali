...
try {
    $stmt = $pdo->query("SELECT * FROM crm_webhook_logs ORDER BY created_at DESC LIMIT 100");
    $logs = $stmt->fetchAll();
...
<div class="apple-container">
    <div class="apple-header">
        <div>
            <h1 class="h3 fw-bold mb-1">سجلات الـ Webhook</h1>
            <p class="text-muted small mb-0">عرض جميع طلبات الـ Webhook</p>
        </div>
                        ...
                        <tbody>
                            <?php foreach ($logs as $log): ?>
                                <tr>
                                    ...
                                    <td><?= h($log['event']) ?></td>
                                    <td>
                                        <span class="apple-badge bg-info"><?= h($log['status']) ?></span>
                                    </td>
