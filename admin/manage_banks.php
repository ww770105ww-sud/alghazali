...
if (isset($_POST['add_bank_account'])) {
    ...
    $bank_branch_number = $_POST['bank_branch_number'] ?? null;
    ...
    try {
        ...
        // 1. العثور على الحساب الرئيسي للبنوك (11102)
        $parent_stmt = $pdo->prepare("SELECT id FROM unified_accounts WHERE account_code = '11102'");
        $parent_stmt->execute();
        $parent_id = $parent_stmt->fetchColumn();
        
        if (!$parent_id) throw new Exception("الحساب الرئيسي للبنوك (11102) غير موجود.");
        ...
        if ($stmt_check_name->fetchColumn() > 0) {
            throw new Exception("اسم البنك موجود بالفعل.");
        }

        // 2. توليد الكود الجديد (11102001, 11102002, ...)
        $stmt_last = $pdo->prepare("SELECT MAX(account_code) FROM unified_accounts WHERE parent_id = ? AND account_code LIKE '11102%'");
        ...
        $stmt = $pdo->prepare("INSERT INTO unified_accounts (account_code, account_name_ar, account_type, normal_balance, parent_id, branch_id, account_status, bank_account_number, bank_branch_address, bank_branch_number) VALUES (?, ?, 'asset', 'debit', ?, ?, ?, ?, ?, ?)");
        $stmt->execute([$new_code, $account_name, $parent_id, $branch_id, $status, $bank_account_number, $bank_branch_address, $bank_branch_number]);
        ...
        $pdo->commit();
        echo "<script>location.href='manage_banks.php?success=1';</script>";
        exit();
    } catch (Exception $e) {
...
if (isset($_POST['update_bank_account'])) {
    ...
    try {
        ...
        $stmt = $pdo->prepare("UPDATE unified_accounts SET account_name_ar = ?, account_status = ?, bank_account_number = ?, bank_branch_address = ?, bank_branch_number = ?, branch_id = ? WHERE id = ?");
        $stmt->execute([$account_name, $new_status, $bank_account_number, $bank_branch_address, $bank_branch_number, $branch_id, $id]);
        
        $pdo->commit();
        echo "<script>location.href='manage_banks.php?success=2';</script>";
        exit();
    } catch (Exception $e) {
...
if (isset($_POST['deactivate_account'])) {
    ...
    try {
        ...
        $stmt->execute([$id]);
        echo "<script>location.href='manage_banks.php?success=3';</script>";
        exit();
    } catch (Exception $e) {
...
if (isset($_POST['delete_account_permanent'])) {
    ...
    try {
        ...
        $pdo->commit();
        echo "<script>location.href='manage_banks.php?success=4';</script>";
        exit();
    } catch (Exception $e) {
        if ($pdo->inTransaction()) $pdo->rollBack();
...
// جلب معرف الأب للبنوك (11102)
$parent_stmt = $pdo->prepare("SELECT id FROM unified_accounts WHERE account_code = '11102'");
$parent_stmt->execute();
$banks_parent_id = $parent_stmt->fetchColumn();

// جلب البنوك من النظام الموحد
$where = "WHERE coa.parent_id = ?";
$params = [$banks_parent_id];
if (!empty($_GET['q'])) {
...
");
$banks_stmt->execute($params);
$banks = $banks_stmt->fetchAll();

// جلب الأرصدة لكل البنوك دفعة واحدة
$bank_ids = array_column($banks, 'id');
$balances = [];
if (!empty($bank_ids)) {
...
<div class="container-fluid py-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h3 class="fw-bold mb-1"><i class="fas fa-university me-2 text-primary"></i> إدارة البنوك</h3>
            <div class="d-flex align-items-center">
                <p class="text-muted small mb-0 me-3">إدارة وتعديل الحسابات البنكية في شجرة الحسابات</p>
        ...
        <button class="btn btn-primary rounded-pill px-4 shadow-sm" data-bs-toggle="modal" data-bs-target="#addBankModal">
            <i class="fas fa-plus-circle me-1"></i> إضافة بنك جديد
        </button>
    </div>
    ...
    <?php if (isset($_GET['success'])): ?>
        <div class="alert alert-success alert-dismissible fade show">
            <?php
            if ($_GET['success'] == 1) echo "تمت إضافة البنك بنجاح.";
            if ($_GET['success'] == 2) echo "تم تحديث بيانات البنك بنجاح.";
            if ($_GET['success'] == 3) echo "تم تحويل البنك إلى خامل بنجاح.";
            if ($_GET['success'] == 4) echo "تم حذف البنك نهائيًا بنجاح.";
            ?>
    ...
    <div class="card border-0 shadow-sm rounded-4">
            ...
            <div class="row align-items-center">
                <div class="col-md-4">
                    <div class="input-group">
                        ...
                        <input type="text" id="bankSearch" class="form-control bg-light border-0" placeholder="بحث سريع باسم أو كود البنك...">
                    </div>
                </div>
        ...
        <div class="card-body p-0">
            <div class="table-responsive">
                <table class="table table-hover align-middle mb-0 text-center" id="banksTable">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td class="px-4">
                                <code class="text-primary fw-bold"><?php echo $bank['account_code']; ?></code>
                            </td>
                            <td>
                                <div class="fw-bold"><?php echo htmlspecialchars($bank['account_name_ar']); ?></div>
                            </td>
                            <td>
                                <span class="badge bg-light text-dark border fw-normal">
                                    <i class="fas fa-building me-1 text-muted"></i>
                                    <?php echo htmlspecialchars($bank['branch_name'] ?? 'عام'); ?>
                                </span>
                            </td>
                            <td><?php echo htmlspecialchars($bank['bank_account_number'] ?? '-'); ?></td>
                            <td><?php echo htmlspecialchars($bank['bank_branch_number'] ?? '-'); ?></td>
                            <td class="small"><?php echo htmlspecialchars($bank['bank_branch_address'] ?? '-'); ?></td>
                            <td>
                                <?php 
                            ...
                            <td>
                                <?php echo get_account_status_label($bank['account_status']); ?>
                            </td>
                            <td>
                                <div class="btn-group">
                                    <a href="account_statement.php?id=<?php echo $bank['id']; ?>" class="btn btn-sm btn-light border-0" title="كشف حساب">
                                        <i class="fas fa-file-invoice-dollar text-primary"></i>
                                    </a>
                                    <button class="btn btn-sm btn-light border-0 edit-bank" 
                                            data-id="<?php echo $bank['id']; ?>" 
                                            data-name="<?php echo htmlspecialchars($bank['account_name_ar']); ?>"
                                            data-branch="<?php echo $bank['branch_id']; ?>"
                                            data-status="<?php echo $bank['account_status']; ?>"
                                            data-account_number="<?php echo htmlspecialchars($bank['bank_account_number'] ?? ''); ?>"
        ...
        <div class="modal-content border-0 shadow-lg rounded-4">
            <form method="POST">
                ...
                <div class="modal-body p-4">
                    <div class="row">
                        <div class="col-md-12 mb-3">
                            <label class="form-label fw-bold small">اسم البنك <span class="text-danger">*</span></label>
                            <input type="text" name="account_name" class="form-control rounded-3" placeholder="مثلاً: بنك التضامن" required>
                        </div>
                        <div class="col-md-12 mb-3">
                        ...
                        <div class="col-md-6 mb-3">
                            <label class="form-label fw-bold small">رقم الحساب البنكي</label>
                            <input type="text" name="bank_account_number" class="form-control rounded-3" placeholder="0000-000000-000">
                        </div>
                        <div class="col-md-6 mb-3">
                            <label class="form-label fw-bold small">رقم الفرع</label>
                            <input type="text" name="bank_branch_number" class="form-control rounded-3" placeholder="مثلاً: 01">
                        </div>
                        <div class="col-md-12 mb-3">
                            <label class="form-label fw-bold small">عنوان الفرع</label>
                            <input type="text" name="bank_branch_address" class="form-control rounded-3" placeholder="مثلاً: صنعاء - شارع حدة">
                        </div>
                        <div class="col-md-6 mb-3">
                ...
                <div class="modal-footer border-0 p-4 bg-light shadow-sm">
                    ...
                    <button type="submit" name="add_bank_account" class="btn btn-primary rounded-pill px-5 fw-bold shadow">حفظ البنك</button>
                </div>
            </form>
        </div>
    </div>
            ...
            <form method="POST">
                ...
                <div class="modal-body p-4">
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            <label class="form-label fw-bold small">رقم الحساب البنكي</label>
                            <input type="text" name="bank_account_number" id="edit_bank_account_number" class="form-control rounded-3">
                        </div>
                        <div class="col-md-6 mb-3">
                            <label class="form-label fw-bold small">رقم الفرع</label>
                            <input type="text" name="bank_branch_number" id="edit_bank_branch_number" class="form-control rounded-3">
                        </div>
                        <div class="col-md-12 mb-3">
                            <label class="form-label fw-bold small">عنوان الفرع</label>
                            <input type="text" name="bank_branch_address" id="edit_bank_branch_address" class="form-control rounded-3">
                        </div>
                        <div class="col-md-12 mb-3">
                            <label class="form-label fw-bold small">الحالة</label>
                ...
                <div class="modal-footer border-0 p-4 bg-light shadow-sm">
                    <button type="button" class="btn btn-white rounded-pill px-4 border" data-bs-dismiss="modal">إلغاء</button>
                    <button type="submit" name="update_bank_account" class="btn btn-warning rounded-pill px-5 fw-bold shadow">حفظ التغييرات</button>
                </div>
            </form>
        </div>
    </div>
